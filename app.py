...
st.set_page_config(
    page_title="포켓몬 도감",
    page_icon="C:/stramlit/images/monsterball.png"  # 경로 확인 필요
)
st.markdown("""
<style>
#css코드 작성하기
...
st.title("streamlit 포켓몬도감")

st.markdown("**포켓몬**을 하나씩 추가하여 도감을 채워보세요!")
...
example_pokemon = {
    "name": "알로라 디그다",
    "types": ["땅", "강철"],
    "image_url": "C:/stramlit/images/알로라_디그다.png"
}

#session state는 일종의 딕셔너리처럼 사용가능
...
with st.form(key = "form"):
    ...
    if submit:
        if not name:
            ...
            st.error("포켓몬의 타입을 적어도 한 개 선택해주세요.")
        else: 
            st.success("포켓몬을 추가할 수 있습니다.")
            st.session_state.pokemons.append({
                "name": name,
                "types":types,
                "image_url":image_url if auto_complete else "C:/stramlit/images/default.png"
            })
